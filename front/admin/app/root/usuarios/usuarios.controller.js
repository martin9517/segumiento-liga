myApp.controller('UsuarioController', function UsuarioController($scope,usuarios,constants) {
    ...
    usuarios.getAll().then(function (data) {
    ...
    $scope.save = function(){
        if($scope.object._id){
            usuarios.update($scope.object).then(function(data){
                usuarios.getAll().then(function(data){
                    $scope.objects = data.data;
                    $scope.pagination.page = 1;
                    $scope.pagination.numPages = Math.ceil($scope.objects.length /$scope.pagination.perPage);
            ...
            usuarios.save($scope.object).then(function(data){
                usuarios.getAll().then(function(data){
                    $scope.objects = data.data;
                    $scope.pagination.page = 1;
                    $scope.pagination.numPages = Math.ceil($scope.objects.length /$scope.pagination.perPage);
    ...
    $scope.remove = function (o) {
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                usuarios.remove(o._id).then(function (data) {
                    usuarios.getAll().then(function (data) {
                        $scope.objects = data.data;
                        swalWithBootstrapButtons.fire(
                            'Eliminado!',
